...
f = plt.figure(facecolor='white', figsize=(15, 8))
...
for i, ads in enumerate(adsorbates):
    ...
    data = np.load(f"../main_MS_GP/run_{ads}/{ads}_rsdata.npy")[()][10]
    ax = plt.subplot(2, 3, i+1, aspect='equal')
    #plt.grid(color='black', linestyle='-', linewidth=0.5)
    #ax.grid(color='grey', linewidth=0.5, alpha=0.01, linestyle='--')
    ax.grid(False)
    plt.xlabel(r"$\Delta E_{DFT} [eV]$", fontsize=18)
    plt.ylabel(r"$\Delta E_{GP} [eV]$", fontsize=18)
    xx = data['y_test']
    yy = data['y_pred_test']
    ...
    plt.colorbar(label='unc. of prediction [eV]', format='%0.2f')
    plt.clim(0.2, 0.9)
...
plt.savefig('GP_acs_test.eps', transparent=True, dpi=1500)
plt.show()
