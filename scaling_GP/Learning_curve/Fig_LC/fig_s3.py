...
for i, ads in enumerate(adsorbates):
    ...
    ca = {'CH': 'C', 'CH2': 'C', 'CH3': 'C', 'NH': 'N', 'SH': 'S', 'OH': 'O'}
    with open(f"../LC_GP/gp_scaling_{ca[ads]}_{ads}/out.txt", "r") as f:
        MAE_train = []
        MAE_train_std = []
        MAE_test = []
        MAE_test_std = []
    ...
    t_d = np.load(f'../LC_GP/gp_scaling_{ca[ads]}_{ads}/{ads}_rsdata.npy')[()]
    num_p = [len(t_d[i][10]['y_train']) for i in t_d]
    del t_d
    ...
    plt.ylabel("MAE [eV]")
    plt.plot(num_p, MAE_train, color='darkgreen', alpha=0.6,
             label='train data', marker='o')
    plt.errorbar(num_p, MAE_train, yerr=MAE_train_std,
                 fmt='.k', color='g', alpha=0.2)
    plt.plot(num_p, MAE_test, color='darkblue', alpha=0.6,
             label='test data', marker='o')
    plt.errorbar(num_p, MAE_test, yerr=MAE_test_std,
...
plt.savefig('delta_LC.eps', transparent=True, dpi=1500)
plt.show()
